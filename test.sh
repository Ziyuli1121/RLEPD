...
# Ad-hoc qualitative comparison script retained for research usage.
FAKE_OUTDIR="${FAKE_OUTDIR:-exps/badbadbad}"
RUN_512_DIR="${RUN_512_DIR:-exps/20251210-005434-sd3_512}"
RUN_1024_DIR="${RUN_1024_DIR:-exps/20251210-011145-sd3_1024}"
...
run_sd3_set() {
  ...
  python sample_sd3.py --predictor "${RUN_512_DIR}/export/network-snapshot-export-step001000.pkl" \
    --seeds "${seeds}" \
    --outdir training_step_images/3 \
    --prompt "${prompt}"

  python sample_sd3.py --predictor "${RUN_512_DIR}/export/network-snapshot-export-step005000.pkl" \
    --seeds "${seeds}" \
    --outdir training_step_images/4 \
    --prompt "${prompt}"

  python sample_sd3.py --predictor "${RUN_512_DIR}/export/network-snapshot-export-step009000.pkl" \
    --seeds "${seeds}" \
...
python sample_sd3.py --predictor "${RUN_1024_DIR}/export/network-snapshot-export-step007200.pkl" \
  --seeds "1" \
...
for steps in 6 7 8 9 12; do
  python sample_sd3_baseline.py --sampler sd3 --resolution 1024 \
    ...
    --seeds "1" \
    --num-steps "${steps}" \
    --outdir "pipeline_images/BAD${steps}"
done
