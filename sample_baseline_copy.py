...
def main(
    ...
    **_
    ...
    trunc_idx_path: Optional[Path] = None
    if prompts_list is not None and hasattr(net.model, "cond_stage_model"):
        cond_stage = net.model.cond_stage_model
        if hasattr(cond_stage, "configure_truncation_logging"):
            trunc_idx_path = Path("./truncated_prompt_indices.txt").resolve()
            if dist.get_rank() == 0:
                if trunc_idx_path.exists():
                    trunc_idx_path.unlink()
                trunc_idx_path.parent.mkdir(parents=True, exist_ok=True)
                trunc_idx_path.touch()
            if torch.distributed.is_initialized():
                torch.distributed.barrier()
            cond_stage.configure_truncation_logging(prompts_list, trunc_idx_path, offset=2)
