            ...
            if guidance_type in ['uncond', 'cfg']:      # LDM and SD models
                with autocast("cuda"):
                    with net.model.ema_scope():
                        teacher_traj = [loss_fn.get_teacher_traj(net=net, tensor_in=latents[k], labels=labels[k], condition=c[k], unconditional_condition=uc[k]) for k in range(num_accumulation_rounds)]
            else:
