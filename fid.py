...
def calculate_inception_stats(
    ...
    loader_kwargs = dict(batch_sampler=rank_batches, num_workers=num_workers)
    ...
    if num_workers > 0:
        ...
        if iterator is not None:
            iterator._shutdown_workers()  # pylint: disable=protected-access
...
@click.option('--workers',              help='Number of dataloader workers', metavar='INT',         type=click.IntRange(min=0), default=0, show_default=True)
@click.option('--prefetch',             help='Prefetch factor when workers>0', metavar='INT',       type=click.IntRange(min=1), default=2, show_default=True)

def calc(image_path, ref_path, num_expected, seed, batch, workers, prefetch):
...
def ref(dataset_path, dest_path, batch, workers, prefetch):
